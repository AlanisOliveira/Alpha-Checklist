...
class ImportarEPCsWidget extends StatefulWidget {
  const ImportarEPCsWidget({
    super.key,
    this.width,
    this.height,
  });
  ...
  @override
  State<ImportarEPCsWidget> createState() => _ImportarEPCsWidgetState();
}

class _ImportarEPCsWidgetState extends State<ImportarEPCsWidget> {
  // Estados do widget
  ...
  // Armazenamentos para agentes (carregados apenas no momento da importação)
  ...
  Future<void> _carregarPrevia() async {
    ...
    try {
      ...
      if (!_validarCabecalhos(linhas[0])) {
        setState(() {
          statusMensagem =
              'Formato de arquivo inválido para EPCs. Verifique o modelo adequado.';
          isLoading = false;
        });
        return;
      }
  ...
  bool _validarCabecalhos(List<dynamic> cabecalhos) {
    ...
    bool temNomeEPC = cabecalhosNormalizados.any((c) =>
        c == 'nome do epc' ||
        c == 'nomeepc' ||
        c == 'nome_do_epc' ||
        c == 'nome');
    ...
    print('Tem nome EPC: $temNomeEPC, Tem ID agente: $temIdAgente');

    // Arquivo é válido se tiver pelo menos as colunas de nome EPC e ID de agente
    return temNomeEPC && temIdAgente;
  }

  // Diálogo de confirmação antes da importação
  Future<bool> _confirmarImportacao() async {
    String mensagem =
        'EPCs novos serão adicionados com seus respectivos agentes.';

    // Adicionar informação sobre duplicados
    switch (opcaoDuplicados) {
      case 'atualizar_inserir':
        mensagem +=
            '\n\nEPCs com nomes já existentes: serão atualizados com os novos dados e relacionamentos.';
        break;
      case 'substituir':
        mensagem +=
            '\n\nEPCs com nomes já existentes: serão atualizados com os novos dados e relacionamentos.';
      ...
      case 'ignorar':
        mensagem +=
            '\n\nEPCs com nomes já existentes: serão ignorados (mantidos como estão).';
        break;
      case 'adicionar':
        mensagem +=
            '\n\nEPCs com nomes já existentes: serão adicionados como novos registros (permitindo duplicatas).';
        break;
    }
  ...
  // Carregar agentes apenas quando necessário para importação
  Future<bool> _carregarAgentesParaImportacao() async {
    setState(() {
      statusMensagem = 'Preparando dados para importação...';
    });
    ...
    try {
      ...
      // Atualizar mapas sem atualizar interface
      mapaAgentes = mapaPorId;
      mapaNomesAgentes = mapaPorNome;

      print('Carregados ${mapaPorId.length} agentes para importação');
      return true;
    } catch (e) {
  ...
  Future<void> _importarDados() async {
    ...
    // Carregar agentes para importação
    if (mapaAgentes.isEmpty) {
      bool sucesso = await _carregarAgentesParaImportacao();
      if (!sucesso) {
        setState(() {
          isLoading = false;
          statusMensagem = 'Falha ao preparar dados para importação.';
        });
        return;
      }
    }
    ...
    try {
      ...
      int epcsAdicionados = 0;
      int epcsAtualizados = 0;
      int ignorados = 0;
      int erros = 0;

      // Identificar colunas para nome do EPC e agentes
      final cabecalhos =
          linhas[0].map((c) => c.toString().trim().toLowerCase()).toList();

      int nomeEPCIndex = cabecalhos.indexWhere((c) =>
          c == 'nome do epc' ||
          c == 'nomeepc' ||
          c == 'nome_do_epc' ||
          c == 'nome');
      ...
      print(
          'Índices: NomeEPC=$nomeEPCIndex, TipoAgente=$tipoAgenteIndex, IdAgente=$idAgenteIndex');
      ...
      for (int i = 1; i < linhas.length; i++) {
        ...
        try {
          ...
          final nomeEPC = linha[nomeEPCIndex].toString().trim();
          if (nomeEPC.isEmpty) {
            print('Linha $i tem nome de EPC vazio');
            erros++;
            continue;
          }
            ...
            if (idsTexto.isNotEmpty) {
              ...
              for (String idStr in idsParts) {
                try {
                  ...
                  // Não verificamos se o ID existe no mapa de agentes para simplificar
                  agenteIds.add(id);
                } catch (e) {
          ...
          if (agenteIds.isEmpty) {
            print('Nenhum agente válido encontrado para o EPC: $nomeEPC');
            erros++;
            continue;
          }

          // Verificar se o EPC já existe para lógica de duplicados
      ...
      setState(() {
        ...
        statusMensagem = 'Importação concluída!\n'
            'EPCs adicionados: $epcsAdicionados\n'
            'EPCs atualizados: $epcsAtualizados\n'
            'Ignorados: $ignorados\n'
            'Erros: $erros';
      });
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text('Importar EPCs'),
        elevation: 2,
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 8),
                  Text(
                    'O arquivo CSV deve conter colunas para: Nome do EPC e ID do Agente.\n\n'
                    'Para associar um EPC a múltiplos agentes, separe os IDs com vírgulas.\n'
                    'Exemplo: "101, 102, 103"',
            ...
            const SizedBox(height: 24),
            Text(
              '3. Como lidar com EPCs já existentes:',
              style: Theme.of(context).textTheme.titleMedium,
            ),
            const SizedBox(height: 8),
            RadioListTile<String>(
              ...
              subtitle: const Text(
                  'Comportamento recomendado: atualiza EPCs existentes e adiciona novos'),
            ...
            RadioListTile<String>(
              ...
              subtitle: const Text(
                  'Atualiza agentes associados aos EPCs com mesmo nome'),
              value: 'substituir',
              groupValue: opcaoDuplicados,
              onChanged: isLoading
                  ? null
            ...
            RadioListTile<String>(
              title: const Text('Ignorar EPCs já existentes'),
              subtitle:
